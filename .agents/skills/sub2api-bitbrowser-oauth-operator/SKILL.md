...
7. Retry a prior `account_banned` reauthorization result only after an explicit
...
8. When phone verification is shown, poll the SMS API in two six-attempt,
...
9. Keep the browser open for review. Only an explicit `--close-window` may
   call the close endpoint; deletion is not implemented.
10. When using the local queue, persist rows only through current-user Windows
...
11. Reset phone cooldowns only on an explicit operator request. Preserve the
   prior use time, reset time, and reset count in the encrypted pool; never
   restore invalid phones through a cooldown reset.
12. Restore an invalid-marked phone only after explicit operator confirmation
...
13. Replace a provider-banned Workstation account only after an explicit
...
14. Treat pending-replacement extraction as secret-bearing. The library may
